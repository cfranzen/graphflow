...
public class SeaFlowEntity extends FlowEntity {
	...
	// // TODO Pfad Abschnitt der Entity für den aktuellen time step als Path2D
	// Objekt berechnen.
	...
	//// addDrawEdge(edge, convertPoint(map, lastCircleP), convertPoint(map,
	// circlePIn), path);
	////
	//// path = new PointPath();
	//// path.moveTo(circlePIn);
	//// path.lineTo(circlePOut);
	//// addDrawEdge(edge, convertPoint(map, circlePIn), convertPoint(map,
	// circlePOut), path);
	//
	// lastPos = pos;
	...
	// */
	// private Point2D getCirclePoint(GeoPosition linePoint, GeoPosition
	// circlePoint) {
